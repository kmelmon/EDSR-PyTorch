...
def GetContainerClient(args):
    if args.client_id != '' :
        try :
            ...
            args.container_client = args.blob_service_client.get_container_client(args.container_name)
...
def save_to_azure(args, filename, tensor_cpu):
    # Create a BytesIO object to hold the image data in memory
    memory_buffer = io.BytesIO()
    imageio.imwrite(memory_buffer, tensor_cpu, format='png')
    png_data = memory_buffer.getvalue()
    memory_buffer.close()        

    try :
        blob_client = args.blob_service_client.get_blob_client(container=args.container_name, blob=filename)
        ...
        exception = e
    if not success :
        print(f'Unable to save:{filename}|Exception:{exception}')            

def bg_target(queue, args):
    GetContainerClient(args)
    while True:
        if not queue.empty():
            filename, tensor = queue.get()
            if filename is None: break
            if (args.save_inference_to_azure != ''):
                filename = args.save_inference_to_azure + '/' + get_filename_from_path(filename)
                save_to_azure(args, filename, tensor.numpy())
            else:
                imageio.imwrite(filename, tensor.numpy())

class checkpoint():
    def __init__(self, args):
        # Ugh!  Generates pickle error due to using thread local storage
        #GetContainerClient(args)
        self.args = args
        self.ok = True
    ...
    def begin_background(self):
        ...
        self.process = [
            Process(target=bg_target, args=(self.queue, self.args)) \
